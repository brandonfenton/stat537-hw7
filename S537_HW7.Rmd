...
G1.k <- numeric(20)
for(j in 1:20){
  kcj <- kmeans(tc2.std, centers=j, iter.max=50, nstart=50)
  G1.k[j] <- index.G1(x=tc2.std,cl=kcj$cluster)
}
...
contingency <- table("Kmeans" = kc3$cluster, "Ward's" = hc3)
colnames(contingency) <- paste("Ward Cluster", 1:3)
rownames(contingency) <- paste("kmeans Cluster", 1:3)
...
```
```{r p8_b, echo=F, comment=NA, message=FALSE, fig.pos="H", fig.align="center", cache=T, size="footnotesize"}
