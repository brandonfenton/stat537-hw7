...
The G1 Index is maximized by the three cluster solution. The maximum is
noticeably higher than the value of the index for two clusters, so it does
not appear that 
...
contingency <- table(hc3, kc3$cluster)
...
plot(tc1$lat~tc1$lon, type="n")
points(x=tc1$lon, y=tc1$lat, pch=20, col=kc3$cluster+3)
```
